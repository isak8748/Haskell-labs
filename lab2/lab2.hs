...
eval (App _ arg) env = error "undefined function"
...
diff v (App "sin" arg) = 
  Op "*" (diff v arg) (App "cos" arg)
diff v (App "cos" arg) = 
  Op "*" (diff v arg) (Op "-" (Const 0) (App "sin" arg))
diff v (App "log" arg) = 
  Op "*" (diff v arg) (Op "/" (Const 1) arg)
diff v (App "exp" arg) = 
  Op "*" (diff v arg) (App "exp" arg)
diff _ _ = error "can not compute the derivative"

simplify :: EXPR -> EXPR
simplify (Const n) = Const n
simplify (Var id) = Var id
simplify (App f arg) = App f (simplify arg)
